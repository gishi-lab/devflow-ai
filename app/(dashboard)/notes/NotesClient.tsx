...
export function NotesClient({ initialNotes }: Props) {
  ...
  // Ref-based error queue: state updates inside startTransition async callbacks
  // can be swallowed by React Concurrent Mode. We queue the error in a ref and
  // flush it in a useEffect so it always reaches the UI.
  ...
  const handleAdd = () => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        pendingErrorRef.current = e instanceof Error ? e.message : "Failed to save note.";
      }
    });
  };
  ...
  const handleUpdate = () => {
    ...
    // Capture the id NOW, before any state updates change selectedNote
    const noteId = selectedNote.id;

    console.log("[handleUpdate] called — noteId:", noteId);
    console.log("[handleUpdate] editValues:", editValues);
    ...
    console.log("[handleUpdate] updated note:", updated);

    // Update UI state immediately (before the async transition)
    ...
    startTransition(async () => {
      // Apply optimistic update so the list reflects changes instantly
      dispatch({ type: "update", note: updated });

      try {
        console.log("[handleUpdate] calling updateNote() with id:", noteId);
        ...
        console.log("[handleUpdate] updateNote() completed successfully");
      } catch (e) {
        const msg = e instanceof Error ? e.message : "Failed to update note.";
        console.error("[handleUpdate] updateNote() threw:", msg, e);
        // Queue via ref so the error survives the transition boundary
  ...
  const handleDelete = (id: string) => {
    ...
    startTransition(async () => {
      ...
      try {
        ...
        pendingErrorRef.current = e instanceof Error ? e.message : "Failed to delete note.";
      }
    });
  };
  ...
  const handleSelectNote = (note: Note) => {
    ...
    console.log("[handleSelectNote] note clicked — id:", note.id, "title:", note.title);
    setSelectedNote(note);
    setShowForm(false);
    setIsEditing(false);
    console.log("[handleSelectNote] setSelectedNote called");
  };

  // ── Render ─────────────────────────────────────────────────────────────────

  // Reusable detail/edit panel content (used both in desktop sidebar and mobile full-screen)
  const detailPanel = selectedNote ? (
    <div className="bg-white rounded-2xl border border-slate-200 p-6">
      {console.log("[detailPanel] rendering for note:", selectedNote.id, selectedNote.title) as unknown as null}
  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      {/* ── Mobile: detail view (replaces list when a note is selected) ── */}
      {selectedNote && (
      ...
      {/* ── Desktop header + list (always visible on lg+); hidden on mobile when note is open ── */}
      <div className={selectedNote ? "hidden lg:block" : ""}>
